...
CHECKPOINT_PATH = Path(r"G:\b1\权重结果\改进实验dfine\dsqc\2e-4_SD18\best_map50.pth")
NUM_CLASSES = 1

# 待预测的小 TIFF 目录，支持递归读取子目录。
INPUT_DIR = Path(r"E:\YOLO\D-FINE\datasets\mydatasets\val\images")

# 对应的 YOLO 标签目录；只推理、不统计漏检误检时设为 None。
LABEL_DIR= Path(r"E:\YOLO\D-FINE\datasets\mydatasets\val\labels")
#LABEL_DIR = None

OUTPUT_DIR = Path(r"G:\b1\权重结果\改进实验dfine\dsqc\2e-4_SD18\验证集切片推理")
